...
OUTPUT_NAME = 'composite_mutual_limiting'
LEGEND_FILENAME = 'composite_legend.png'

# *** CRITICAL SETTING ***
# Identify your TARGET map (e.g., 'w6ek' or 'site_1')
TARGET_KEYWORD = 'N5JEF Base' 
...
class MapLayer:
    def __init__(self, kml_path):
        ...
        self.is_target = TARGET_KEYWORD.lower() in self.base_name.lower()
...
def create_legend_image(color_scale):
    ...
    for db, r, g, b in color_scale.entries:
        ...
        draw.rectangle([total_width - box_width, current_y, total_width, current_y + box_height], fill=(r, g, b, 255), outline=(0, 0, 0, 255))
...
def create_composite_map():
    ...
    for kml in kml_files:
        ...
        try:
            layer = MapLayer(kml)
            layers.append(layer)
            role = "TARGET" if layer.is_target else "Network"
            print(f" - Found [{role}]: {layer.base_name}")
            if layer.is_target: target_found = True
        except Exception: pass

    if not layers: return
    if not target_found:
        print(f"WARNING: No file matched TARGET_KEYWORD '{TARGET_KEYWORD}'.")
        return
    ...
    # 1. Best Target Signal (Initialize to Infinity)
    target_best_loss = np.full((master_h, master_w), UNDEFINED_PATH_LOSS, dtype=np.float32)
    
    # 2. Best Network Signal (Initialize to Infinity)
    # This will hold the STRONGEST signal from any NON-TARGET map.
    network_best_loss = np.full((master_h, master_w), UNDEFINED_PATH_LOSS, dtype=np.float32)

    for layer in layers:
        print(f"Reading {layer.base_name}...")
        try:
            with Image.open(layer.png_path) as img:
                ...
                # --- SORT INTO BUFFERS ---
                if layer.is_target:
                    # Keep the lowest (strongest) dB for the target
                    dest_slice = target_best_loss[y_start:y_end, x_start:x_end]
                    better_mask = temp_loss < dest_slice
                    target_best_loss[y_start:y_end, x_start:x_end] = np.where(better_mask, temp_loss, dest_slice)
                else:
                    # Keep the lowest (strongest) dB for the network
                    dest_slice = network_best_loss[y_start:y_end, x_start:x_end]
                    better_mask = temp_loss < dest_slice
                    network_best_loss[y_start:y_end, x_start:x_end] = np.where(better_mask, temp_loss, dest_slice)

        except Exception as e:
            print(f"Error: {e}")

    # --- CALCULATE MUTUAL LIMITING LINK ---
    print("Calculating Limiting Link...")
    
    # Step 1: "Take the weakest of (our target point & the strongest mutual point)"
    # Weakest = MAXIMUM dB value (High dB is bad)
    # Strongest Network Point is already stored in 'network_best_loss' (Min dB)
    limiting_loss = np.maximum(target_best_loss, network_best_loss)

    # Step 2: "As long as they are both stronger than threshold"
    # If limiting_loss <= 150, then by definition both Target and Network are <= 150.
    valid_mask = limiting_loss <= VALID_THRESHOLD_DB
    ...
    for db, r, g, b in color_scale.entries:
        # Match pixels where limiting_loss == db
        mask = (limiting_loss == db) & valid_mask
        
        final_rgba[mask, 0] = r
        final_rgba[mask, 1] = g
        final_rgba[mask, 2] = b
        final_rgba[mask, 3] = 255
    ...
    kml_lines = [
        ...
        '<Folder>',
        f'  <name>Limiting Link Coverage: {TARGET_KEYWORD}</name>',
        '  <GroundOverlay>',
        '    <name>Limiting Link Map</name>',
